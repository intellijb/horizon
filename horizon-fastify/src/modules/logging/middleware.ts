...
function createRequestContext(request: FastifyRequest): LogContext {
  ...
  const traceId = 
    (request.headers[TRACE_ID_HEADER] as string) ||
    generateCorrelationId();
...
function createRequestLogData(request: FastifyRequest): RequestLogData {
  ...
  return {
    ...
    params: Object.keys(request.params as object).length > 0 ? request.params : undefined,
    body: request.body,
  };
}
...
function createResponseLogData(request: FastifyRequest, reply: FastifyReply): ResponseLogData {
  ...
  return {
    ...
    headers: loggingConfig.responseEnabled ? reply.getHeaders() : undefined,
  };
}
