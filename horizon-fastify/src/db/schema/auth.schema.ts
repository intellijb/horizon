...
export const accessTokens = authSchema.table('access_tokens', {
  id: uuid('id').defaultRandom().primaryKey(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  deviceId: uuid('device_id').notNull().references(() => devices.id, { onDelete: 'cascade' }),
  ...
  userDeviceIdx: index('access_tokens_user_device_idx').on(table.userId, table.deviceId),
...
export const oauthAccounts = authSchema.table('oauth_accounts', {
  ...
  providerUnique: unique('oauth_accounts_provider_user_unique').on(table.provider, table.providerUserId),
}));

// Authentication attempts table
export const authAttempts = authSchema.table('auth_attempts', {
  id: uuid('id').defaultRandom().primaryKey(),
  email: text('email'),
